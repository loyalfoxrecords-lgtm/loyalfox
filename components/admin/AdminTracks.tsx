...
function TrackForm({ data, onChange, onUpload, uploading }: {
  data: typeof empty;
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:"16px" }}>
      ...
      <div>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:"12px" }}>
          ...
          {data.image_url && (
            <p style={{ fontSize:"11px", color:css.green, fontFamily:"system-ui" }}>✓ Imagen subida</p>
          )}
        </div>
      </div>
    </div>
              ...
              <>
                <tr key={track.id} style={{ borderBottom:`1px solid ${css.border}`,
                  ...
                  <td style={{ padding:"10px 16px" }}>
                    <p style={{ fontSize:"13px", fontWeight:500, color:css.text, fontFamily:"system-ui" }}>{track.name}</p>
                    <p style={{ fontSize:"11px", color:css.textMute, fontFamily:"system-ui" }}>{track.duration}</p>
                  </td>
                  <td style={{ padding:"10px 16px", fontSize:"13px", color:css.textSub, fontFamily:"system-ui" }}>{track.artist}</td>
                  <td style={{ padding:"10px 16px" }}>
                    {track.genre && <span style={hs.badgeGray}>{track.genre}</span>}
                  </td>
                  <td style={{ padding:"10px 16px", fontSize:"13px", color:css.textSub, fontFamily:"system-ui" }}>
                      ...
                      {track.url_spotify && (
                        <a href={track.url_spotify} target="_blank" rel="noreferrer"
                          style={{ fontSize:"11px", color:css.green, fontFamily:"system-ui",
                            textDecoration:"none", padding:"2px 6px", background:css.greenBg,
                            borderRadius:"4px", border:`1px solid ${css.greenBorder}` }}>Spotify</a>
                      )}
                      {track.url_apple && (
                        <a href={track.url_apple} target="_blank" rel="noreferrer"
                          style={{ fontSize:"11px", color:css.textSub, fontFamily:"system-ui",
                            textDecoration:"none", padding:"2px 6px", background:css.bg,
                            borderRadius:"4px", border:`1px solid ${css.border}` }}>Apple</a>
                      )}
                      {track.url_beatport && (
                        <a href={track.url_beatport} target="_blank" rel="noreferrer"
                          style={{ fontSize:"11px", color:css.blue, fontFamily:"system-ui",
                            textDecoration:"none", padding:"2px 6px", background:css.blueBg,
                            borderRadius:"4px", border:`1px solid ${css.blueBorder}` }}>Beatport</a>
                      )}
                    </div>
                  </td>
                  <td style={{ padding:"10px 16px" }}>
                    <div style={{ display:"flex", gap:"6px" }}>
                      ...
                      <button onClick={() => deleteTrack(track.id)} style={hs.btnDanger}>
                        Eliminar
                      </button>
                    </div>
                  </td>
