...
export default function StreamerOverlay({ token }: { token: string }) {
  ...
  const nextTrack = () => {
    setTracks(prev => {
      ...
      if (shuffle) {
        ...
        next = (currentIndex + 1) % prev.length;
      }
      setCurrentIndex(next);
      setCurrentTrack(prev[next]);
      return prev;
    });
    setProgress(0);
  };
  ...
  const prevTrack = () => {
    setTracks(prev => {
      if (prev.length === 0) return prev;
      const prev2 = (currentIndex - 1 + prev.length) % prev.length;
      setCurrentIndex(prev2);
      setCurrentTrack(prev[prev2]);
      return prev;
    });
    setProgress(0);
  };
  ...
  return (
    <div style={{ background:"transparent", minHeight:"100vh",
      ...
      {/* Selector de playlist — solo visible antes de empezar */}
      {showSelector && (
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(160px,1fr))",
            ...
            {playlists.map((pl) => (
              <div key={pl.id} onClick={() => loadPlaylist(pl.id)}
                ...
                <p style={{ fontSize:"10px", letterSpacing:"1px", textTransform:"uppercase",
                  color:"#e8e8e8", textAlign:"center" }}>{pl.name}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ position:"fixed", bottom:"24px", left:"24px", zIndex:50 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:"0",
            ...
            <div style={{ flex:1, padding:"10px 14px", overflow:"hidden" }}>
              ...
              <p style={{ fontSize:"13px", color:"#e8e8e8", letterSpacing:"0.5px",
                overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap",
                marginBottom:"2px", fontFamily:"'Bebas Neue',sans-serif", lineHeight:1.2 }}>
                {currentTrack.name}
              </p>
              ...
              {/* Barra de progreso */}
              <div style={{ marginTop:"8px", height:"2px", background:"rgba(255,255,255,0.1)",
      ...
      {/* Lista de tracks — lateral derecha, scrollable */}
      {!showSelector && tracks.length > 0 && (
        <div style={{ position:"fixed", right:"24px", top:"50%",
          ...
          <div style={{ padding:"10px 14px", borderBottom:"1px solid rgba(57,255,20,0.1)" }}>
          ...
          {tracks.map((tr, i) => (
            <div key={tr.id} onClick={() => selectTrack(tr, i)}
              ...
              {tr.image_url ? (
                ...
                <div style={{ width:"32px", height:"32px", background:"#1a1a1a", flexShrink:0 }} />
              )}
              <div style={{ overflow:"hidden", flex:1 }}>
                <p style={{ fontSize:"10px", color: currentIndex === i ? "#39ff14" : "#e8e8e8",
                  overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap",
                  marginBottom:"2px" }}>{tr.name}</p>
